...
\( \frac{V_L dt}{L} = di\).  Here, \(dt\) is the time in which voltage is rising, or the product of the duty cycle \(k\) and the switching period \(T_s\), so \(di = \frac{V_L kT_s}{2L}\) since the total current rise is twice the ripple current.  Substituting our circuit parameters, we arrive at a ripple current of 0.67A.  Adding this ripple value to the average value for this point gives a maximum inductor current of 1.92A, which approaches the saturation value of 2.3A for the indcutor in use in the circuit.  While it is dubious whether the inductor actually enters saturation, it is the best explanation I current have.
